export class Bus {
  ...
  subscribe (chanel, callback) {
    ...
    this.subscribers[chanel] = {subscription}
  ...
  send (chanel, message) {
    if (chanel in this.subscribers) {
      console.log(this.subscribers[chanel])
      this.subscribers[chanel].subscription.messages.push(message)
      // проход по всем подпискам канала и отправка сообщения
      for (const callback of this.subscribers[chanel].subscription) {
        callback(message)
      }
    }
  }
  ...
  getMessages (chanel) {
    if (chanel in this.subscribers) {
      console.log("1")
      return this.subscribers[chanel].messages
    }
  }
